...
function AddHotel({ inputs, title, type }) {
   ...
   is used to fetch data from the API endpoint `'https://backend-vr9r.onrender.com/rooms'` and
   update the state variable `roomData` with the response data. */
    useEffect(() => {
        const roomsss = async () => {
            const room = await axios.get('https://backend-vr9r.onrender.com/rooms');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post('https://backend-vr9r.onrender.com/hotel/create', newHotel);
            setLoading(false);
            nevigate(`/hotels`);
        } catch (error) {
            console.log(error);
